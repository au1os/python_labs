...
#### Декоратор класса @dataclass
Автоматически генерирует конструктор __init__() для инициализации полей


Методы __repr__(), __eq__() для сравнения объектов
#### Поля класса
fio(str): ФИО студента


birthdate (str): Дата рождения в формате "DD.MM.YYYY"
...
#### Методы и их логика
##### __post_init__(self)
Дополнительная валидация после инициализации объекта:
...
Преобразует строку birthdate в объект date

Вычисляет разницу между текущей датой и датой рождения

Корректирует возраст, если день рождения в текущем году еще не наступил
##### to_dict(self) -> dict
...
Возвращает словарь с ключами, соответствующими полям класса
##### @classmethod from_dict(cls, d: dict)
Десериализация словаря в объект (фабричный метод)
...
Создает новый объект Student, передавая значения из словаря
...
### Теория
#### Структуры данных, их операции и сложности
...
Сложности (реализация на массиве/списке):
...
Поиск элемента: O(n) (не базовая операция)
...
enqueue(x) — добавление в конец очереди

dequeue() — удаление из начала очереди

front() — просмотр первого элемента

isEmpty() — проверка на пустоту

Сложности (реализация на связном списке):
...
Связный список - структура данных, которая состоит из Узлов. Каждый узел хранит значение, и ссылку/и на след/след и пред элементы списка, хранящиеся не упорядочено.

Основные операции:

insertAtHead(x) — вставка в начало

insertAtTail(x) — вставка в конец

deleteAtHead() — удаление из начала

deleteNode(key) — удаление по значению
...
Вставка в конец: O(n) (без tail-указателя), O(1) (с tail-указателем)
...
#### Какие структуры быстрее, какие медленнее
Самый быстрый: Стек (на массиве)

Почему:

Все операции O(1)

Данные хранятся последовательно в памяти (cache-friendly)
...
Также быстрый: Очередь (на массиве/кольцевом буфере)
Почему:

Все операции O(1)

Данные также хранятся последовательно

Сложность реализации выше стека, но скорость сравнима


Медленнее: Связный список
Почему медленнее:

Cache misses: элементы разбросаны в памяти

Overhead: на каждый элемент 2 указателя (8-16 байт)

Поиск за O(n): произвольный доступ медленный

Аллокация памяти: каждый новый элемент — новое выделение памяти
### structures.py
```python
...
```
![test](./images/lab10/structures.png)
### linked_list.py
```python
class Node:
...
print(f"__len__() --> {len(sll)} (3)")
```
![test](./images/lab10/linked_list.png)